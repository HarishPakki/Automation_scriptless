...
function ExcelEditor() {
  ...
  const handleSave = () => {
    const fs = window.require('fs'); // ✅ safely load fs in Electron
    const path = localStorage.getItem('excelPath');
  
    if (!path) {
      alert('Original file path not found!');
      return;
    }
    ...
    const wbout = XLSX.write(wb, { bookType: 'xlsx', type: 'buffer' }); // ✅ get buffer
    fs.writeFileSync(path, wbout); // ✅ overwrite original file
  
    alert('Changes saved back to original Excel file successfully!');
  };
